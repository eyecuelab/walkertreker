...
class Start extends React.Component {
  constructor(props) {
    super(props);
    const { notification } = this.props.screenProps;
    this.state = {
      notification
    };
  }
  ...
  componentDidMount = async () => {
    ...
    if (this.state.needCampaign) {
      dispatch({
        ...
        id: this.state.localPlayer.campaignId
      });
    }
  ...
  navigate = route => {
    const { path } = this.props.screenProps;
    let routeName = route;
    let routeParams = {};
    if (this.state.notification) {
      const { type } = this.state.notification.data;
      if (navigation[type] !== "none") {
        routeName = navigation[type];
      }
      routeParams = this.state.notification.data.data
        ? { data: this.state.notification.data.data }
        : {};
    }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.player.id === null && this.props.player.id !== null) {
      this.setState({ gotPlayer: true }); // eslint-disable-line react/no-did-update-set-state
    }
    if (prevProps.campaign.id === null && this.props.campaign.id !== null) {
      this.setState({ gotCampaign: true }); // eslint-disable-line react/no-did-update-set-state
    }
    if (
      this.state.needPlayer === this.state.gotPlayer &&
      this.state.needCampaign === this.state.gotCampaign
    ) {
      if (this.state.needCampaign) {
  ...
  render() {
    return (
      <ImageBackground
        source={require("../../assets/splash2.png")} // eslint-disable-line global-require
