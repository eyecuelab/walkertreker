...
class EndOfDaySummary extends React.Component {
    ...
    if (this.state.didWeMakeIt) {
      return (
        <Text style={styles.label}>
          The evening comes, and you are safe... for now.
        </Text>
      );
    }
    return (
      <Text style={styles.label}>
        Night falls upon the safehouse... but not all of you are here...
      </Text>
    );
  }
  ...
  _footerTextRender() {
    ...
    return (
      <Text style={[styles.plainText, { marginTop: 25, marginBottom: 20 }]}>
        {text}
      </Text>
    );
  }
            ...
            <View style={customStyles.playersContainer}>
              ...
              >
                {this.state.campaign.players.map(player => {
                  const playerUpdate = this.state.update.players.filter(
