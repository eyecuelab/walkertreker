...
class CampaignStaging extends React.Component {
  constructor(props) {
    ...
    this.state = {
      invites, // eslint-disable-line react/no-unused-state
      selectedPlayer: "none",
    ...
    if (this.props.campaign.numPlayers === 1) {
      return (
        <TwoButtonOverlay
          ...
          button1onPress={() =>
            console.log("Not enough players to launch game")
          }
          button2title="Invite"
  ...
  _toggleWhenToStartModal = () => {
    const { whenToStartModalVisible } = this.state;
    this.setState({ whenToStartModalVisible: !whenToStartModalVisible });
  };
      ...
      <View style={{ backgroundColor: "#871C1D" }}>
        ...
        >
          <Modal isVisible={this.state.whenToStartModalVisible}>
            <WhenToStartForm
              handleModalStateChange={this._toggleWhenToStartModal}
            />
          </Modal>
          ...
          <ScreenContainer>
            <View style={customStyles.contentContainer}>
              ...
              <View style={customStyles.panelContainer}>
                <ScrollView
                  contentContainerStyle={customStyles.scrollContainer}
                  showsVerticalScrollIndicator
                >
                  <SubHeader>Players</SubHeader>
                  <View
                    style={[
                      customStyles.contactListContainer,
                      customStyles.contactListFirst
                    ]}
                  >
                    <PlayersList onSelectPlayer={this._toggleSelectedPlayer} />
                  </View>

                  <View
                    style={[
                      customStyles.contactListContainer,
                      customStyles.contactListSecond
                    ]}
                  >
