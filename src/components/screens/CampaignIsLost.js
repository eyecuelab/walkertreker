...
class CampaignIsLost extends React.Component {
  constructor(props) {
    ...
    const { finalCampaignState, causeOfDeath, deadPlayers } = data;
  ...
  deadPlayersPlainText() {
    const names = [];
    this.state.deadPlayers.forEach(player => {
    ...
    if (names.length === 1) {
      string = `${names[0]}${
        this.state.causeOfDeath === "beaten" ? " was" : ""
      }`;
    } else {
      ...
      string = `${string} and ${names[names.length - 1]}${
        this.state.causeOfDeath === "beaten" ? " were" : ""
      }`;
    }
    return string;
  }
  ...
  endGameText() {
    ...
    if (this.state.causeOfDeath === "starved") {
      text = `After so many long days of running and fighting and low on energy and resources, on day ${this
        .state.finalCampaignState.currentDay +
        1}, your party is forced to make a difficult decision. Weak with hunger and unable to continue, you are forced to leave ${this.deadPlayersPlainText()} behind, knowing full well what this means. Part of staying alive in such a situation also means having to live with yourself on days like this.`;
    }
